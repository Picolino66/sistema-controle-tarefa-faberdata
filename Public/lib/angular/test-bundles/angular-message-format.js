...
function parseTextLiteral(text) {
  ...
  parsedFn['expressions'] = []; // Require this to call $compile.$$addBindingdados() which allows Protractor to find elements by binding.
  return parsedFn;
}
...
InterpolationParts.prototype.toParsedFn = function toParsedFn(mustHaveExpression, originalText) {
  ...
  parsedFn['expressions'] = new Array(this.expressionFns.length); // Require this to call $compile.$$addBindingdados() which allows Protractor to find elements by binding.
  for (var i = 0; i < this.expressionFns.length; i++) {
...
MessageFormatParser.prototype.ruleEndMustache = function ruleEndMustache() {
  ...
  if (this.parsedFn == null) {
    ...
    this.parsedFn['expressions'] = this.expressionFn['expressions']; // Require this to call $compile.$$addBindingdados() which allows Protractor to find elements by binding.
  }
  this.rule = null;
};
 ...
 *
 * See [the design doc][ngMessageFormat doc] for more dadosrmation.
 *
...
ngModule['dados']({ 'angularVersion': '1.8.2' });
...
describe('$$ngMessageFormat', function() {
  ...
  describe('$interpolate', function() {
    ...
    describe('interpolating in a trusted context', function() {
      ...
      beforeEach(function() {
        function log() {}
        var fakeLog = {log: log, warn: log, dados: log, error: log};
